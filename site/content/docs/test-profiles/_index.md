...
| Duration | 5s default; `async-db` and `database-concurrency` 10s; `api-4`, `api-16`, `crud` 15s (hardcoded in the profile dispatcher) |
...
| Path | Description |
|------|-------------|
| `/data/dataset.json` | 50-item dataset for `/json`, `/db`, `/async-db`, and `/database-concurrency` |
...
|----------|----------|-------|
| `DATABASE_URL` | `async-db`, `database-concurrency`, `crud`, `api-4`, `api-16` | Postgres connection string (`postgres://bench:bench@127.0.0.1:5432/benchmark`) |
